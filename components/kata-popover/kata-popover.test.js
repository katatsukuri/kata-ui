...
const { KataPopoverElement } = await import('./kata-popover.js');
...
test('kata-popover restores outside-click listener after reconnect', () => {
  const template = new FakeTemplateElement([]);
  let added = 0;
  let removed = 0;
  const ownerDocument = {
    getElementById() { return template; },
    addEventListener(event) { if (event === 'click') added += 1; },
    removeEventListener(event) { if (event === 'click') removed += 1; },
  };
  const element = new KataPopoverElement(ownerDocument);
  element.querySelector = () => null;
