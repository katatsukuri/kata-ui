...
Katatsukuri UI（技術識別子: `kata-ui`）は、長期保守するWebアプリケーションへ段階的に導入できる、ビルドレスでframework-agnosticなWeb Components UIライブラリです。UIの内部構造をopen Shadow DOMに閉じ、利用側は属性とslotだけでコンポーネントへデータを渡します。

特にserver-driven applicationを主対象とし、アプリケーション状態や業務ロジックをブラウザ側へ移すことなく、Web標準に基づく再利用UIの境界を提供します。ReactやVueのようなクライアント側アプリケーション基盤を置き換えることは目的としていません。HTMXは、サーバーを業務状態の正本に保ったままHTMLを部分更新するための推奨統合手段の一つです。
...
## Katatsukuri UIが有用な理由

Katatsukuri UIは、業務システム全体のアプリケーション基盤ではなく、再利用する価値があるUIだけをWeb Componentとして切り出します。

| Katatsukuri UIの方針 | 業務システムにもたらす効果 |
| --- | --- |
...
| HTML更新手段、Page Runtime、Web Componentの責務分離 | 通信、画面状態、部品状態の所有者を分け、二重描画と状態の二重管理を避ける |
...
一方、Katatsukuri UIは業務ルール、認証・認可、サーバー側入力検証、画面固有レイアウト、主要業務フローのE2Eを代替しません。これらを利用アプリケーションの責務として残すことが、再利用可能なUI境界を小さく保つための条件です。
...
Katatsukuri UIは次の分担を採用します。HTML更新が必要な場合は利用アプリケーションが手段を選択し、HTMXを利用するときの具体的な統合契約は後述します。

| 状態・処理 | 所有者 |
| --- | --- |
| 業務状態、認証、認可、永続データ | サーバー |
| サーバー通信とHTML差し替え | 利用アプリケーション（HTMXなど） |
