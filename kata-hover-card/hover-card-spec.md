...
2. `<template>` の内容はトリガー要素（`data-hover-card-trigger` 属性を持つ要素）とコンテンツ要素（`data-hover-card-content` 属性を持つ要素）を含むこと
...
| `data-hover-card-content` | ホバーカードのコンテンツ領域 |
| `.hover-card-content` | コンテンツのスタイル用クラス |
...
<template id="kata-hover-card-template">
  ...
  <div data-hover-card-content class="hover-card-content">
    <p class="hover-card-title">山田太郎</p>
    <p class="hover-card-description">2023年1月から利用中</p>
  </div>
</template>
