...
`data` 属性（JSON）または `setData()` メソッドでデータを渡すと、内部に `<canvas>` を生成してグラフを描画します。

## 必須条件

1. `type` 属性でグラフ種別を指定すること（未指定時は `bar`）
2. `data` 属性に Chart.js の `data` オブジェクト（JSON 文字列）を渡すこと
3. ページに Chart.js が読み込まれていること（`window.Chart` または `import` で解決）
...
<link rel="stylesheet" href="kata-chart/kata-chart.css">
...
<script type="module" src="kata-chart/kata-chart.js"></script>
```
